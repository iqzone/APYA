...
_temp.prototype.template_editor = {
	...
	*/
	initialize: function()
	{
		...
		// Doesn't work in Safari, but appears to be Event.observe related, rather than the event itself.
		// @link	http://scoop.simplyexcited.co.uk/2007/12/14/prototype-script-onbeforeunload-when-leaving-a-page/ (read comments)
		window.onbeforeunload	= function()
		{
			...
			if( changes )
			{
				return ipb.lang['templates_navigate'];
			}
		};
	},
	...
	*/
	buildTemplateList: function( group, json )
	{
		...
		for( i = 0 ; i < json['templates'].length; i++ )
		{
			...
			else if ( json['templates'][i]['template_user_edited'] == 1 ) /*&& json['templates'][i]['template_user_added'] == 0*/ /* Bug 25788 */
			{
				/* This is a modified row */
				state  = 'modified';
			}
	...
	*/
	removeFile: function( e, file )
	{
		...
		if( file['template_id'] )
		{
			...
			var url = "app=core&module=ajax&section=templates&do=revertTemplateBit&setID=" + this.currentSetData['set_id'] + "&template_id=" + acp.template_editor.checkMap( fileid, 'template' ) + '&secure_key=' + ipb.vars['md5_hash'];
		...
		else
		{
			...
			var elemprefix = "css_";
			var url        = "app=core&module=ajax&section=css&do=revertCSS&fromDelete=1&setID=" + acp.template_editor.currentSetData['set_id'] + "&css_id=" + acp.template_editor.checkMap( fileid, 'css' ) + '&secure_key=' + ipb.vars['md5_hash'];
		...
		new Ajax.Request( url.replace(/&amp;/g, '&' ),
						{
							...
							onSuccess: function( t )
							{
								...
								if( file['template_id'] )
								{
									...
									{
										var _oldId	= acp.template_editor.reverseCheckMap( fileid, 'template' );
										
										if( _oldId )
										{
											if( !Object.isUndefined( acp.template_editor.currentlyOpen.get('template_' + _oldId) ) ){
												acp.tabbedEditor.closeTab( 'template_' + _oldId );
												return true;
											}
										}
									}
		...
		new Ajax.Request( url.replace(/&amp;/g, '&' ),
						{
							...
							onSuccess: function( t, e )
							{
								...
								{		
									...
									{
										acp.template_editor.modifyMap['template'][ t.responseJSON['templateData']['template_id'] ]  = undefined; // Remove old map
								...
								else
								{
									...
									{
										acp.template_editor.modifyMap['css'][ t.responseJSON['cssData'][ info['css_group'] ]['css_id'] ]  = t.responseJSON['cssData'][ info['css_group'] ]['css_id']; // Remove old map
		...
		if( $('t_revert') )
		{
			...
			try {
				if( type == 'template' )
				{
					...
					if( !Object.isUndefined( info ) && $('group_' + info['template_group'] + '_bit_' + fileid) )
					{
		...
		new Ajax.Request( url.replace( /&amp;/g, '&' ),
						  {
							...
							onSuccess: function (t)
							{
								...
								{
									acp.template_editor.modifyMap['css'][ t.responseJSON['cssData']['css_id'] ] = t.responseJSON['cssData']['css_id']; // Remove old map
		...
		new Ajax.Request( url.replace( /&amp;/g, '&' ),
						  {
							...
							onSuccess: function (t, e)
							{
								...
								if( type == 'template_' ){
									...
									if( acp.template_editor.checkMap( fileid, 'css' )!= t.responseJSON['cssData']['css_id'] )
									{
										acp.template_editor.modifyMap['css'][ t.responseJSON['cssData']['css_id'] ] = undefined; // Remove old map
										acp.template_editor.modifyMap['css'][ fileid ] = t.responseJSON['cssData']['css_id'];
										Debug.write("Updated modify map!");
									}
	...
	*/
	compareTemplate: function(e)
	{
		...
		popup = new ipb.Popup( 'comparison', { type: 'pane', modal: true, w: ( _dims.width - 50 ) + 'px', h: ( _dims.height - 150 ), ajaxURL: url, hideAtStart: false, close: 'a[rel="close"]' }, { 'afterAjax': _callback } );
		return false;
	},
