...
class public_blog_actions_dorate extends ipsCommand
{
	/**
	* Blog id
	*
	...
	*/
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		{
			$this->registry->output->showError( 'no_guests', 10623, null, null, 404 );
		}
		...
		{
			$this->registry->output->showError( 'blog_allready_rated', 10624 );
		}
		...
		if ( empty($rating) or $rating < 1 or $rating > 5 or $this->memberData['member_id'] == $entry['entry_author_id'] )
		{
			$this->registry->output->showError( 'incorrect_use', 10625 );
		}
		
		/* Got a previous rating? */
		if ( $entry['rating_id'] != '' )
		{
			$this->DB->delete( 'blog_ratings', "member_id=" . $this->memberData['member_id'] . " and blog_id=" . $this->blog['blog_id'] . " AND entry_id=" . $entry_id );
		}
		
		$insert_rate = array( 'member_id'	=> $this->memberData['member_id'],
