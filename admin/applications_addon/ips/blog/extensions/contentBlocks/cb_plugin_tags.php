...
*/
class cb_tags extends contentBlocks implements iContentBlock
{
	...
	{
		$return_html = "";
		$return_html = $this->registry->getClass('blogFunctions')->fetchTagCloud( $this->blog['blog_id'], $this->blog );
		
		if( $return_html or $this->settings['blog_inline_edit'] )
		{
			$return_html = $this->registry->output->getTemplate( 'blog_cblocks' )->cblock_header( $cblock, $this->lang->words['cblock_get_my_tags'] ).$return_html;
