...
event_recurring, FROM_UNIXTIME(event_unix_from), CASE WHEN event_unix_to > 0 THEN FROM_UNIXTIME(event_unix_to) ELSE NULL END, '', 0, 0, 0, 0, MD5( CONCAT( event_id, event_title ) ), 0, 1 FROM {$PRE}cal_events_bak;";

$SQL[] = "DROP TABLE cal_events_bak;";

/* Add fulltext indexes, if appropriate */

if( $DB->checkFulltextSupport() )
{
	$SQL[] = "ALTER TABLE cal_events ADD FULLTEXT (event_content);";
	$SQL[] = "ALTER TABLE cal_events ADD FULLTEXT (event_title);";
}
